...
TYPE_ALIASES = {
    ...
    "мест в законодательном органе": "legislature_seats",
...
ALIASES = {
    ...
    "преемник": "successor",
    "партия": "party",
    "всего мест": "total_seats",
    "порог большинства": "majority",
    "созыв": "term",
    "период": "term",
    "партии и места": "parties",
    "партии": "parties",
    "тип графика": "chart_type",
    "данные графика": "data_points",
    "данные": "data_points",
    "ось x": "x_label",
    "подпись оси x": "x_label",
    "ось y": "y_label",
    "подпись оси y": "y_label",
    "единица измерения": "unit",
    "пункты сравнения": "items",
    "кандидаты": "candidates",
    "партии / кандидаты": "candidates",
    "явка": "turnout",
    "события": "events",
    "группы / доли": "items",
...
def infer_type(keys: set[str]) -> str:
    ...
    scores = {
        ...
        "parliament": 3 * len(keys & {"total_seats", "parties", "majority"}),
        "chart": 3 * len(keys & {"chart_type", "data_points", "x_label", "y_label"}),
        "comparison": 3 * len(keys & {"items", "unit"}),
        "election": 3 * len(keys & {"candidates", "turnout"}),
        "timeline": 3 * len(keys & {"events"}),
        "composition": 2 * len(keys & {"items", "subtitle"}),
    }
